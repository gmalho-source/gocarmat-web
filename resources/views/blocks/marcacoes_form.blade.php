...
<section class="mt-7 overflow-hidden rounded-[32px] bg-energia px-8 py-14 sm:px-12 xl:px-24 xl:py-[100px]">
    ...
    <h1 class="mt-6 text-5xl font-bold leading-[1.1] tracking-[-0.03em] text-white sm:text-6xl 2xl:text-7xl">
        {{ $data['titulo'] ?? 'Marcações' }}
    </h1>

    @if (session('success'))
        <div class="mt-10 max-w-[900px] rounded-2xl bg-lima px-8 py-6">
            <p class="text-lg font-bold text-carbono">{{ $data['sucesso_titulo'] ?? 'Pedido enviado com sucesso!' }}</p>
            <p class="mt-1 text-base text-carbono">{{ $data['sucesso_texto'] ?? 'Recebemos a sua marcação e enviámos um e-mail de confirmação. A nossa equipa entrará em contacto consigo brevemente.' }}</p>
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('marcacoes.store') }}" class="mt-12 max-w-[1100px]">
        ...
        <div class="mt-8 space-y-4">
            <label class="flex items-start gap-3 text-base font-light text-gelo">
                ...
                {{ $data['newsletter_texto'] ?? 'Quero subscrever a newsletter GOCARMAT e receber dicas e campanhas.' }}
            </label>
            <label class="flex items-start gap-3 text-base font-light text-gelo">
                <input type="checkbox" name="privacy" value="1" required class="mt-1 size-5 shrink-0 accent-lima">
                <span>{{ $data['rgpd_texto'] ?? 'Aceito e dou o meu consentimento para a recolha e tratamento dos meus dados pessoais (RGPD), usados exclusivamente pela GOCARMAT para responder a este pedido.' }} <a href="{{ url('/politica-de-privacidade') }}" class="underline hover:text-white">Política de Privacidade</a>. *</span>
            </label>
        </div>
